...
entity somador_tb is
    generic (
        HALF_PERIOD_PS : integer := 50000; -- default 50 ns 
        WAIT_TIME_NS   : integer := 500   -- default 5000 ns
    );
end somador_tb;
...
begin
	...
	--clk <= not clk after 100 ns;	-- 10 MHz: 100 ns;  500 MHz: 2 ns
	
	-- Generates the stimuli
    rst_n <= '0', '1' after (HALF_PERIOD_PS * 1 ps)*0.5;
    
    -- Multiply the integer by 1 ps to create a VHDL time physical type
    clk <= not clk after (HALF_PERIOD_PS * 1 ps);
			...
			while NOT (endfile(imageFile)) loop	-- Main loop to read the file
				...
				readline(imageFile, fileLine);	
					...
					if a_i="00000000" and b_i="00000000" then
                        -- Multiply the integer by 1 ns
                        wait for (WAIT_TIME_NS * 1 ns);
                    end if;
			end loop;


			std.env.finish; -- Forces the simulator to exit cleanly  	
